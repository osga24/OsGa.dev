...
const getViewportScale = (viewportWidth: number) =>
  clamp(viewportWidth / 430, 0.02, 1);
...
const toFontSize = (
  ...
  const viewportScale = getViewportScale(viewportWidth);
  const progressRatio = LAST_STEP === 0 ? 1 : value / LAST_STEP;
  const dynamicScale = lerp(lengthScale, 1, progressRatio);
  const dynamicViewportScale = lerp(viewportScale, 1, progressRatio);
  const adjustedSize = size * dynamicScale * dynamicViewportScale;
...
export const initWordmarkIntro = ({
  ...
  if (
    ...
    if (wordmark instanceof HTMLElement) {
      ...
      wordmark.style.fontSize = toFontSize(
        LAST_STEP,
        finalText.length,
        viewportWidth,
      );
  ...
  wordmark.style.fontSize = toFontSize(0, finalText.length, viewportWidth);
  ...
  timeline.to(wordmark, {
    ...
    ease: "power3.out",
    fontSize: toFontSize(LAST_STEP, finalText.length, viewportWidth),
    onUpdate() {
