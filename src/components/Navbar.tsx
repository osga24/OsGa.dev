...
const Navbar = () => {
  ...
  return (
    ...
    >
            ...
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span>{item.name}</span>
                  <span className="absolute bottom-0 left-0 w-full h-0.5 bg-green-400 transform scale-x-0 transition-transform duration-300 group-hover:scale-x-100" />
                </Link>
              );
            })}
          </div>
      ...
      {/* Mobile Navigation */}
      <div
        className={`md:hidden absolute w-full backdrop-blur-md transition-all duration-300 ease-in-out overflow-hidden
		${isOpen ? "max-h-96 opacity-100 bg-black/70" : "max-h-0 opacity-0 bg-transparent"}`}
      >
        <div
          className={`px-4 py-3 transform transition-all duration-300 ease-in-out ${
          ...
          {navigation.map((item, index) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`group flex items-center space-x-3 px-4 py-3 rounded-lg
				text-gray-300 hover:text-white hover:bg-white/5 text-base font-medium transform transition-all
				duration-300 ease-in-out ${isOpen ? "translate-x-0 opacity-100" : "translate-x-8 opacity-0"}`}
                style={{
